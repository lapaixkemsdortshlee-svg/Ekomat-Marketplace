...
- **Exécuter le pilote fermé (COURT TERME, PRIORITAIRE — l'audit est fait, place au terrain).** Protocole complet : `docs/PROTOCOLE-PILOTE-FERME.md` (2026-07-17). Restent 3 actions, toutes côté Thrasher : (1) **G2 : configurer un provider SMS** (Supabase → Auth → Providers → Phone, Twilio/Vonage — décision 2026-07-17 : OTP vendeur OBLIGATOIRE au pilote) ; (2) **G3 : test OTP réel** sur un numéro Digicel/Natcom ; (3) **recruter 2-3 vrais vendeurs contrôlés** + 1-2 acheteurs contrôlés par vendeur. Critère de succès : ≥3 transactions `released` sur ≥2 vendeurs → enchaîner sur le closed testing Google Play. Cadrage Thrasher inchangé : « pas de démo, préparer l'infra pour de vrais résultats ». Le goulot n'est plus technique.
